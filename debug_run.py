...
def _test_data(tmp_dir):
    ...
    try:
        ...
        check("Admin DefaultAdmin seeded",       "DefaultAdmin" in admins)
        check("No techs on fresh init",    techs == [])
        D.init_db()  # idempotency
        check("Init is idempotent",        D.db_list_users()[0] == ["DefaultAdmin"])
        ...
        check("Remove only admin refused", D.db_remove_user("DefaultAdmin") is False)
        D.db_add_user("SecondAdmin", "admin", "7410")
        check("Remove DefaultAdmin when 2nd admin exists", D.db_remove_user("DefaultAdmin") is True)
