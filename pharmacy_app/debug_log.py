...
def trace(fn):
    """Decorator: log function entry and completion without values at DEBUG.
    ...
    def wrapper(*args, **kwargs):
        if _log.isEnabledFor(logging.DEBUG):
            _log.debug(
                "enter %s positional=%d keyword=%d",
                fn.__name__,
                len(args),
                len(kwargs),
            )
        ...
        if _log.isEnabledFor(logging.DEBUG):
            _log.debug("exit %s result_type=%s", fn.__name__, type(result).__name__)
        return result
